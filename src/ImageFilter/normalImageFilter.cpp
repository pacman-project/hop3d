        ...
        for (size_t row=0; row<octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo].size();row++){
            for (size_t col=0; col<octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row].size();col++){
                if (octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col].get()!=nullptr){
                        ...
                        for (int i=0;i<3;i++){
                            for (int j=0;j<3;j++){
                                if (octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col]->partIds[i][j]>=0){
                                    ...
                                    if (i==1&&j==1)
                                        vdp.locationEucl = octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col]->partsPosNorm[1][1].mean.block<3,1>(0,0);
                                    else
                                        vdp.locationEucl = octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col]->partsPosNorm[1][1].mean.block<3,1>(0,0)+octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col]->partsPosNorm[i][j].mean.block<3,1>(0,0);
                    ...
                    if (octetsImages[layerNo][overlapNo][categoryNo][objectNo][imageNo][row][col]->secondOctet.size()>0){
                        for (int i=0;i<3;i++){
                            for (int j=0;j<3;j++){
